...
|----|--------|-------------|------|--------|------|
| 1 | email | VARCHAR(255) | NO | - | 이메일 (기본키, 로그인 ID) |
| 2 | email_verified | BOOLEAN | NO | FALSE | 이메일 인증 여부 |
| 3 | password_hash | VARCHAR(255) | YES | - | 비밀번호 해시 |
| 4 | roles_code | VARCHAR(10) | NO | '002' | 역할 코드 FK → user_roles.code |
...
| 3 | code | VARCHAR(10) | NO | - | 역할 코드, UNIQUE, PK |
...
(FK: user_email) (FK: user_email_fk)              (FK: user_email)
    │               │                              user_change_logs
    │               │                              (FK: user_email, changed_by_email)
    │ N:1           │ N:1                          admin_logs
    ▼               ▼                              (FK: admin_email)
price_plans ◄───────┘
    │
...
user_roles ◄── users (N:1, FK: roles_code → code)
```
